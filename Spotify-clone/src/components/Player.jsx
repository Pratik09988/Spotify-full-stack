...
function Player() {
    ...
    const QueueSong = ({ song, label, isCurrent = false }) => {
        ...
        return (
            <div className={`flex items-center gap-3 p-2 rounded-lg transition ${isCurrent ? "bg-[#333]" : "hover:bg-[#2d2d2d]"}`}>
                <img src={song.image} alt="" className="w-11 h-11 rounded object-cover" />
                ...
                <span className={`text-[10px] ${isCurrent ? "text-green-500" : "text-gray-500"}`}>
                    {label}
                </span>
            </div>
        );
    ...
    return (
        <div className="h-[10%] bg-black flex justify-between items-center text-white px-4">
            <div onClick={handleCurrentSongClick} className={`hidden lg:flex items-center gap-4 min-w-[220px] ${track.album ? "cursor-pointer hover:bg-[#ffffff12] rounded-lg p-1 transition" : "cursor-default"}`}>
            ...
            <div className="flex flex-col items-center gap-1 m-auto">
                <div className="flex gap-4 items-center">
                    <FontAwesomeIcon onClick={toggleShuffle} icon={faShuffle} className={`w-4 cursor-pointer ${shuffle ? "text-green-500 opacity-100" : "opacity-70"}`} />
                    <FontAwesomeIcon onClick={previous} icon={faBackwardStep} className="w-4 cursor-pointer hover:text-green-500" />
                    {playStatus ? (
                        <FontAwesomeIcon onClick={pause} icon={faPause} className="w-4 cursor-pointer hover:text-green-500" />
                    ) : (
                        <FontAwesomeIcon onClick={play} icon={faPlay} className="w-4 cursor-pointer hover:text-green-500" />
                    )}
                    <FontAwesomeIcon onClick={next} icon={faForwardStep} className="w-4 cursor-pointer hover:text-green-500" />
                    <FontAwesomeIcon onClick={toggleRepeat} icon={faRepeat} className={`w-4 cursor-pointer ${repeatMode !== "off" ? "text-green-500 opacity-100" : "opacity-70"}`} />
                </div>
            ...
            <div className="hidden lg:flex items-center gap-3 opacity-75">
                <FontAwesomeIcon onClick={toggleNowPlaying} icon={faPlayCircle} className={`w-4 cursor-pointer hover:text-green-500 ${showNowPlaying ? "text-green-500" : ""}`} />
                <FontAwesomeIcon onClick={toggleQueue} icon={faList} className={`w-4 cursor-pointer hover:text-green-500 ${showQueue ? "text-green-500" : ""}`} />
                <FontAwesomeIcon onClick={toggleMute} icon={isMuted || volume === 0 ? faVolumeXmark : volume < 0.5 ? faVolumeLow : faVolumeHigh} className={`w-4 cursor-pointer hover:text-green-500 ${isMuted ? "text-red-500" : ""}`} />
                <div onClick={changeVolume} className="w-20 h-1 bg-gray-500 rounded-full cursor-pointer relative">
                    <div className="absolute left-0 top-0 h-1 bg-green-500 rounded-full" style={{ width: `${volume * 100}%` }} />
                </div>
                <FontAwesomeIcon onClick={toggleFullscreen} icon={faDisplay} className="w-4 cursor-pointer hover:text-green-500" />
            </div>
            {showNowPlaying && (
                <div className="fixed bottom-[10%] right-4 w-80 bg-[#242424] rounded-lg p-5 z-50 shadow-xl">
                    <img src={track.image} alt="" className="w-full rounded mb-4" />
            ...
            {showQueue && (
                <div className="fixed bottom-[10%] right-4 w-80 bg-[#242424] rounded-lg p-4 z-50 shadow-xl">
