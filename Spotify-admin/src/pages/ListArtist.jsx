...
const ListArtist = () => {
  ...
  const fetchArtists = async () => {
    try {
      ...
      if (response.data.success) {
        ...
        toast.error(response.data.message || "Unable to load artist data");
      }
    } catch (error) {
      console.log(error);
  ...
  const removeArtist = async (id) => {
    try {
      ...
      if (response.data.success) {
        ...
        toast.error(response.data.message || "Unable to remove artist");
      }
    } catch (error) {
      console.log(error);
      toast.error(error.response?.data?.message || error.message);
  ...
  return (
    <div className="w-full min-w-0">
      <h2 className="text-lg sm:text-xl font-semibold text-center text-[#173C27] mb-4 underline">All Artist List</h2>
      {data.length === 0 ? (
        <div className="bg-[#FFFDF7] border border-[#D9DED6] rounded-xl p-8 text-center text-[#68766E]">No artists found</div>
      ) : (
        <>
          <div className="hidden md:block overflow-x-auto bg-[#FFFDF7] rounded-xl border border-[#D9DED6] shadow-sm">
            ...
            {data.map((item) => {
              const artistColor = item.bgColour || item.bgColor || item.color || "#ffffff";
              return (
                <div key={item._id} className="min-w-[760px] grid grid-cols-[80px_1.1fr_2fr_1.2fr_70px] items-center gap-3 px-4 py-3 border-t border-[#E1E5DF] text-sm text-[#274433] hover:bg-[#F5F8F1] transition">
                  <img className="w-12 h-12 object-cover rounded-full" src={item.image} alt={item.name} />
                  <p className="font-medium truncate">{item.name}</p>
                  <p className="truncate" title={item.desc}>{item.desc}</p>
                  <div className="flex items-center gap-2 min-w-0">
                    <div className="w-9 h-9 rounded-lg border border-[#CCD4CC] shrink-0" style={{ backgroundColor: artistColor }} />
                    <span className="text-xs truncate">{artistColor}</span>
                  </div>
                  <button type="button" onClick={() => removeArtist(item._id)} className="w-9 h-9 rounded-lg flex items-center justify-center text-red-600 border border-red-200 hover:bg-red-50 transition">
              ...
              return (
                <div key={item._id} className="bg-[#FFFDF7] border border-[#D9DED6] rounded-xl p-4 shadow-sm">
                  <div className="flex items-start gap-3">
                    ...
                    <button type="button" onClick={() => removeArtist(item._id)} className="w-9 h-9 rounded-lg flex items-center justify-center text-red-600 border border-red-200 hover:bg-red-50 shrink-0">
                      <FontAwesomeIcon icon={faTrash} />
                    </button>
                  </div>
                  <div className="flex items-center gap-3 mt-4 pt-3 border-t border-[#E5E8E3]">
                    <div className="w-8 h-8 rounded-lg border border-[#CCD4CC]" style={{ backgroundColor: artistColor }} />
                    <div>
                      <p className="text-[10px] text-[#718477]">Background Colour</p>
                      <p className="text-xs font-medium text-[#274433]">{artistColor}</p>
                    </div>
                  </div>
                </div>
              );
